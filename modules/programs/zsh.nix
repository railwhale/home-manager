...
let
  ...
  zdotdir = if cfg.dotDir == null then
    config.home.homeDirectory
    # Check if path is absolute. If it isn't,
    # then use old method of making zdotdir
  else if (builtins.substring 0 1 cfg.dotDir) == "/" then
    cfg.dotDir
  else
    "${config.home.homeDirectory}/${cfg.dotDir}";
  ...
  finalZshenvStr = concatStringsSep "\n" [
    (''
    ...
    (optionalString (cfg.dotDir != null) ''export ZDOTDIR="${zdotdir}"'')
  ...
  finalZshrcStr = concatStringsSep "\n" ([
    ...
    (optionalString (localVarsStr != "") localVarsStr)
    ...
    (optionalString
      (cfg.completion.enable && !cfg.oh-my-zsh.enable && !cfg.prezto.enable) ''
        # Completion setup
        ${cfg.completion.init}'')
    ...
    ''
      ${if cfg.autocd != null then
        ...
        ""}
    ''
    (optionalString (cfg.initExtra != "") cfg.initExtra)

    (optionalString (cfg.shellAliases != "") ''
      # Aliases
      ${aliasesStr}'')

    (optionalString (cfg.initExtraAfterAliases != "") cfg.initExtraAfterAliases)

  ] ++ (mapAttrsToList
    (k: v: "alias -g -- ${lib.escapeShellArg k}=${lib.escapeShellArg v}")
    cfg.shellGlobalAliases) ++ [
      (optionalString (cfg.dirHashes != { }) ''
...
in {
  ...
  options = {
    programs.zsh = {
      ...
      dotDir = mkOption {
        ...
        description = ''
          Absolute path to the directory where the zsh configuration
          and more should be located. The default is the home directory.
        '';
      };
      ...
      initExtraAfterAliases = mkOption {
        default = "";
        type = types.lines;
        description =
          "Extra commands that should be added to {file}`.zshrc`, after aliases are set.";
      };
  ...
  config = mkIf cfg.enable (mkMerge [
    # When cfg.dotDir is set, only use ~/.zshenv to source ZDOTDIR/.zshenv,
    ...
    # For all shells
    {
      home.file."${zdotdir}/.zshenv".text = finalZshenvStr;
    }

    # For login shells: e.g. via tty/ssh (before .zshrc)
    (mkIf (cfg.profileExtra != "") {
      home.file."${zdotdir}/.zprofile".text = cfg.profileExtra;
    })

    # For interactive shells (like when you open a terminal)
    {
      home.file."${zdotdir}/.zshrc".text = finalZshrcStr;
    }

    # For login shells: e.g. via tty/ssh (after .zshrc)
    (mkIf (cfg.loginExtra != "") {
      home.file."${zdotdir}/.zlogin".text = cfg.loginExtra;
    })

    # Run when logging out of a login shell
    (mkIf (cfg.logoutExtra != "") {
      home.file."${zdotdir}/.zlogout".text = cfg.logoutExtra;
    })

    {
      home.packages = [ cfg.package ]
        ++ optional cfg.completion.enable pkgs.nix-zsh-completions;
    }

    (mkIf cfg.oh-my-zsh.enable {
      home.packages = [ cfg.oh-my-zsh.package ];
